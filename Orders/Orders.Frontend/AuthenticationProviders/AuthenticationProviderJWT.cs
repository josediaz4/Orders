...
namespace Orders.Frontend.AuthenticationProviders
{
    public class AuthenticationProviderJWT : AuthenticationStateProvider, ILoginService
    {
        ...
        private readonly string _tokeyKey;
        ...
        {
            var token = await _jSRuntime.GetLocalStorage(_tokeyKey);
            if (token == null)
            {
                return _anonimous;
            }
        ...
        {
            await _jSRuntime.SetLocalStorage(_tokeyKey, token);
        ...
        {
            await _jSRuntime.RemoveLocalStorage(_tokeyKey);
